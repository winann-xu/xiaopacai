...
## 2026-08-14 上午：熄屏是否计时实测（结论：不计时）
- 真机两轮实验：基线 712 分钟 → 熄屏 2分35秒 / 70秒 → 亮屏强制采集仍 712，无增长。
- 使用时长来自系统 UsageStatsManager 前台使用事件，熄屏后前台应用停止，不计时。
- 首轮 710→712 的 +2 是熄屏前已累计数据的采集延迟，非熄屏计时。
- 注意：该真机「今日总时长」明显偏大（9:40 已 710 分钟），疑似系统 UsageStats 历史
  累积异常（与之前模拟器假数据同源），与熄屏计时无关。
