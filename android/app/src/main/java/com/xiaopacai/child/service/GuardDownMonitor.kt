 ...
 *
 * 职责：
 * 1. 记录守护失效开始/结束时间与失守时长（本地持久化 + P2P 上报家长端）；
...
object GuardDownMonitor {
    ...
    /** 发送单条守护事件（P2P guard_event 消息） */
    private fun sendEvent(context: Context, event: JSONObject): Boolean {
        return try {
            val p2p = GuardianForegroundService.getP2PConnection()
            if (p2p.connectionState.value != P2PConnectionState.CONNECTED) return false
            val message = P2PMessage(
                type = "guard_event",
                payload = mapOf("guardEvent" to event.toString())
            )
